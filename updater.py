...
def patch_updater(src):
    ...
    if old_sort not in src:
        raise RuntimeError('backend analyst-first sort marker missing')
...
def patch_index(src):
    ...
    if old not in src:
        raise RuntimeError('frontend ticker-search sort marker missing')
    ...
    if old not in src:
        raise RuntimeError('frontend featured sort marker missing')
    src = src.replace(old, new, 1)
    return src
...
def main():
    ...
    for token in [
        '_extract_analyst_details_v16_core',
        'AI optimism',
        'news.sort(key=lambda z: z.get("ts", ""), reverse=True)',
    ]:
        if token not in updater:
            raise RuntimeError(f'v17-fixed self-test missing: {token}')
    if 'const ap=(a.analystPriority?1:0)' in index:
        raise RuntimeError('old analyst-first frontend search sort still present')
    if 'items=all.filter(x=>x.featured!==false).slice(0,18);' in index:
        raise RuntimeError('old unsorted featured feed still present')

    UPDATER.write_text(updater, encoding='utf-8')
    INDEX.write_text(index, encoding='utf-8')
    print(f'Final updater installed: {len(updater)} bytes, newsPatch=v17-fixed')
    print('Frontend installed: newest-first news feed + ticker search')
